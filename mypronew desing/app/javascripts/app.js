...
window.App = {
  ...
  $("#transfer-amount").click(function(event) {
    ...
    var self = this;
    var address1 = document.getElementById("t").value;
    ...
    Bank.deployed().then(function(instance) {
      return instance.transfer(address1,{from: account, gas: 6000000});
    }).then(function() {
      $("#status").html("Transaction complete!");
      App.showBalance();
    }).catch(function(e) {
      ...
      return instance.show_registers();
       ...
       $.each(val,function(err,data){
        bank.bank_d1(data).then(function(result){
          $("#body_bank").append('<tr><td>'+data+'</td><td>'+result[0]+'</td><td>'+web3.fromWei(result[1].toNumber(), "ether")+'</td><td>'+result[3]+'</td><td>'+result[4]+'</td><td>'+result[5]+'</td></tr>')
        })
       })
    ...
    return bank.fix_bank_count(account);
      ...
      for(var i=0;i<val.toNumber();i++){
        bank.fix_bank(account,i).then(function(data,err){
          ...
          $("#fixed_list").append('<tr><td>'+data[0]+'</td><td>'+data[1]+'</td><td>'+data[3]+'</td></tr>');
        });
      }
  });
},
    ...
    return bank.fix_user_count(account);
      ...
      for(var i=0;i<val.toNumber();i++){
        bank.fix_user(account,i).then(function(data,err){
          $("#fixed_loan_list").append('<tr><td>'+data[0]+'</td><td>'+data[1]+'</td><td>'+data[2]+'</td><td>'+data[3]+'</td><td>'+data[4]+'</td><td>'+data[5]);
        });
      }
  });
},
